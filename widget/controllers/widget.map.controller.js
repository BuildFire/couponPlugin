...
(function (angular, buildfire, window) {
  angular.module("couponPluginWidget").controller("WidgetMapCtrl", [
    "$scope",
    "DataStore",
    "TAG_NAMES",
    "LAYOUTS",
    ...
    function (
      ...
      WidgetMap.yesterdayDate = +WidgetMap.currentDate.setDate(
      ...
      WidgetMap.getAllItems = function (filter) {
        ...
        if (WidgetMap.isFilterApplied) {
          ...
          if (filter.categories && filter.categories.length) {
            ...
            itemFilter = {
              "$json.SelectedCategories": { $in: filter.categories },
            };
      ...
      WidgetMap.setRedeemedItems = function () {
        for (var item = 0; item < WidgetMap.locationData.items.length; item++) {
          ...
          for (var redeem in WidgetMap.redeemed) {
            if (
              WidgetMap.locationData.items[item].id ==
              WidgetMap.redeemed[redeem].data.itemId
            ) {
              WidgetMap.locationData.items[item].isRedeemed = true;
              WidgetMap.locationData.items[item].redeemedOn =
                WidgetMap.redeemed[redeem].data.redeemedOn;
            }
          }
        }
      ...
      WidgetMap.formatItems = function () {
        ...
        for (var i = 0; i < WidgetMap.locationData.items.length; i++) {
          for (var j = i + 1; j < WidgetMap.locationData.items.length; j++) {
            if (
              !WidgetMap.locationData.items[i].alreadySet &&
              !WidgetMap.locationData.items[j].alreadySet &&
              angular.equals(
                WidgetMap.locationData.items[i].data.location,
                WidgetMap.locationData.items[j].data.location
              )
            ) {
              isChanged = true;
              ...
              if (WidgetMap.locationData.items[i].couponContained.length == 0)
                WidgetMap.locationData.items[i].couponContained.push(
                  angular.copy(WidgetMap.locationData.items[i])
                );
              WidgetMap.locationData.items[i].couponContained.push(
                angular.copy(WidgetMap.locationData.items[j])
              );
            }
          }
        }
      ...
      WidgetMap.setSavedItem = function () {
        ...
        for (var item = 0; item < WidgetMap.locationData.items.length; item++) {
          ...
          for (var save in WidgetMap.saved) {
            if (
              WidgetMap.locationData.items[item].id ==
              WidgetMap.saved[save].data.itemId
            ) {
              isChanged = true;
              WidgetMap.locationData.items[item].isSaved = true;
              WidgetMap.locationData.items[item].savedId =
                WidgetMap.saved[save].id;
            }
          }
        }
      ...
      WidgetMap.init = function (cb) {
        ...
        var success = function (result) {
            ...
            if (!WidgetMap.data.design.itemListLayout) {
              WidgetMap.data.design.itemListLayout =
                LAYOUTS.itemListLayout[0].name;
            }
            if (!WidgetMap.data.content) WidgetMap.data.content = {};
          ...
          error = function (err) {
            Buildfire.spinner.hide();
            WidgetMap.data = {
              design: { itemListLayout: LAYOUTS.itemListLayout[0].name },
            };
        ...
        if (typeof Storage !== "undefined") {
          ...
          if (userLocation) {
            WidgetMap.locationData.currentCoordinates = JSON.parse(
              userLocation
            );
          } else getGeoLocation(); // get data if not in cache
        } else {
        ...
        GeoDistance.getDistance(
          ...
          function (result) {
            if (
              ...
              WidgetMap.selectedItemDistance =
                result.rows[0].elements[0].distance.text;
            } else {
      ...
      WidgetMap.addRemoveSavedItem = function (item, multipleCoupons, index) {
        if (item.isSaved && item.savedId) {
          ...
          UserData.delete(
            ...
            WidgetMap.currentLoggedInUser._id
      ...
      var onUpdateCallback = function (event) {
        setTimeout(function () {
          ...
          if (event && event.tag === TAG_NAMES.COUPON_INFO) {
            ...
            if (!WidgetMap.data.design) WidgetMap.data.design = {};
            if (!WidgetMap.data.content) WidgetMap.data.content = {};
            if (!WidgetMap.data.settings) WidgetMap.data.settings = {};
            ...
            function (result) {
              ...
              for (var _ind = 0; _ind < endIndex; _ind++) {
                if (_items && _items[_ind]) {
                  _items[_ind].data.distance =
                    result.rows[0].elements[_ind].status != "OK"
                      ? -1
                      : result.rows[0].elements[_ind].distance.value;

                  if (
                    WidgetMap.isFilterApplied &&
                    WidgetMap.filter.distanceRange
                  ) {
                    var itemDistNo =
                      result.rows[0].elements[_ind] &&
                      result.rows[0].elements[_ind].distance &&
                      result.rows[0].elements[_ind].distance.text &&
                      Number(
                        result.rows[0].elements[_ind].distance.text
                          .split(" ")[0]
                          .replace(/,/g, "")
                    ...
                    var distanceUnit =
                      ...
                      result.rows[0].elements[_ind].distance &&
                      result.rows[0].elements[_ind].distance.text &&
                      result.rows[0].elements[_ind].distance.text.split(" ")[1];
                    var filterDistMin = WidgetMap.filter.distanceRange.min;
                    var filterDistMax = WidgetMap.filter.distanceRange.max;
                    var sortFilterCond;
                    if (
                      (distanceUnit == "km" && filterDistMax > 483) ||
                      (distanceUnit == "mi" && filterDistMax > 300)
                    )
                      sortFilterCond = itemDistNo >= filterDistMin;
                    else
                      sortFilterCond =
                        itemDistNo >= filterDistMin &&
                        itemDistNo <= filterDistMax;
